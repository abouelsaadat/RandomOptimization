...
# Six Peaks Cost Function (Baluja and Caruana, 1995)
def sixpeaks(input_x, T):
    # Calculate R(input, T)
    R_count = 0
    if (
        tail(0, input_x) > T
        and head(1, input_x) > T
        or tail(1, input_x) > T
        and head(0, input_x) > T
    ):
        R_count = len(input_x)
    else:
        R_count = 0
    ...
    return (
        max(tail(0, input_x), head(1, input_x), tail(1, input_x), head(0, input_x))
        + R_count
    )
...
def problem_eval_function(input):
    return sixpeaks(input, ENTRY_T_PEAK)
